#include "JNI/jni.h"
#include <string>
#include "JNI/JNIHelp.h"
#include "poly/poly.h"
#include "poly/poly_clip.cpp"
#include <math.h>
#include "ETC1/etc1.h"
#include "ETC1/etc1.cpp"
...
static const JNINativeMethod gtestEtc1Methods[] = {
...
static const ClassRegistrationInfo gClasses2[] = {
    {"ohos/opengl/Matrix", gMatrixMethods, NELEM(gMatrixMethods)},
    {"ohos/opengl/Visibility", gVisibilityMethods, NELEM(gVisibilityMethods)},
    {"ohos/opengl/ETC1", gtestEtc1Methods, NELEM(gtestEtc1Methods)},
};
...
Java_ohos_opengl_ETC1_one(JNIEnv* env) {//注册
    nativeClassInitBuffer(env);
    for (int i = 0; i < NELEM(gClasses2); i++) {
    const ClassRegistrationInfo* cri = &gClasses2[i];
    env->RegisterNatives(env->FindClass(cri->classPath), cri->methods, cri->methodCount);
}
    return 1;
}
